...
GLuint vertexBuffer, uvBuffer, normalBuffer, texture;
...
void initOpenGLProgram(GLFWwindow* window) {
	...
	if (!loadOBJ("templo.obj")) { // Załaduj plik .obj
		std::cerr << "Failed to load OBJ file" << std::endl;
		exit(EXIT_FAILURE);
	}
	initBuffers(); // Zainicjuj bufory OpenGL
	texture = readTexture("marmur.png");
}
...
void drawScene(GLFWwindow* window, float angle_x, float angle_y) {
	...
	glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer);
	glVertexAttribPointer(spTextured->a("vertex"), 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

	glEnableVertexAttribArray(spTextured->a("normal"));
	glBindBuffer(GL_ARRAY_BUFFER, normalBuffer);
	glVertexAttribPointer(spTextured->a("normal"), 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

	glEnableVertexAttribArray(spTextured->a("texCoord0"));
	glBindBuffer(GL_ARRAY_BUFFER, uvBuffer);
	glVertexAttribPointer(spTextured->a("texCoord0"), 2, GL_FLOAT, GL_FALSE, 0, (void*)0);
	...
	glDisableVertexAttribArray(spTextured->a("texCoord"));  //Wyłącz przesyłanie danych do atrybutu texCoord0
